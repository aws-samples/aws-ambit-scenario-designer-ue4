...
namespace AmbitFileHelpers
{
    FString LoadSingleFileFromPopup(const FString& AllowedFileTypes)
    {
        ...
        bool bSuccessfullyOpenedFile = DesktopPlatform->OpenFileDialog(
        ...
        const FString& DefaultPath, const FString& Filename)
    {
        ...
        TArray<FString> OpenFilenames;
        bool bSuccessfullyOpenedFile = DesktopPlatform->SaveFileDialog(
            FSlateApplication::Get().FindBestParentWindowHandleForDialogs(nullptr),
    ...
    FString GetPathForDirectoryFromPopup(const FString& DefaultPath)
    {
        ...
        FString OutFolder;
        bool bSuccessfullyOpenedFile = DesktopPlatform->OpenDirectoryDialog(
            FSlateApplication::Get().FindBestParentWindowHandleForDialogs(nullptr),
    ...
    FString CompressFile(const FString& SourceDir, const FString& TargetDir, const FString& FileName, const FString& TargetPlatform) {
        int32 ReturnCode = 0;
        ...
        if (TargetPlatform == "LinuxNoEditor") {
            ...
            if (Platform == "Windows") {
                Executable = FPaths::Combine(*FPlatformMisc::GetEnvironmentVariable(TEXT("ProgramW6432")), TEXT("7-Zip"), TEXT("7z.exe"));
                if (FPaths::FileExists(Executable)) {
                    Arguments = "a " + Destination + " " + FPaths::Combine(*SourceDir, TEXT("*")) + " -tzip -mmt -mx5";
                }
                else {
                    Executable = "powershell";
                    Arguments = "-command \"& { Compress-Archieve -Update -Path " + FPaths::Combine(*SourceDir, TEXT("*"))
        ...
        FProcHandle ProcHandle = FPlatformProcess::CreateProc(*Executable, *Arguments, false, false, false, NULL, 0, NULL, NULL);
        if (!ProcHandle.IsValid())
        {
            ErrorMessage = "Failed to create process with command " + Executable + " " + Arguments + ".";
            throw std::runtime_error("Compress file " + std::string(TCHAR_TO_UTF8(*DestinationFile)) + " Failed. " + std::string(TCHAR_TO_UTF8(*ErrorMessage)));
        }
        FPlatformProcess::WaitForProc(ProcHandle);
        FPlatformProcess::GetProcReturnCode(ProcHandle, &ReturnCode);

        if (ReturnCode != 0) {
            ErrorMessage = "Command " + Executable + " " + Arguments + " failed with error code " + FString::FromInt(ReturnCode) + ".";
            throw std::runtime_error("Compress file " + std::string(TCHAR_TO_UTF8(*DestinationFile)) + " Failed. " + std::string(TCHAR_TO_UTF8(*ErrorMessage)));
        }
