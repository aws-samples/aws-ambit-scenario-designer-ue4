...
void SpawnWithHoudiniSpec::Define()
{
    ...
    Describe("GenerateObstacles() and PopulateParameters()", [this]()
    {
        Describe("When no HDA Selected", [this]()
        {
            ...
            It("Should return no spawned actors with configuration that has surface actor name when MatchBy is NameOrTags", [this]()
            {
            ...
            It("Should return no spawned actors with configuration that has surface actor name and tag when MatchBy is NameAndTags", [this]()
            {
        ...
        Describe("When one HDA is selected", [this]()
        {
            ...
            It("Should return SpawnedActors when configuration has surface actor name when MatchBy is NameOrTags", [this]()
            {
                Spawner->GenerateObstacles();
            ...
            It("Should not return SpawnedActors when no configuration doesn't have surface actor name when MatchBy is NameOrTags", [this]()
            {
        ...
        Describe("When multiple HDA are selected", [this]()
        {
            ...
            It("Will have the correct output for item's SpawnedActors when multiple HoudiniAssetDetails are valid", [this]()
            {
                Spawner->GenerateObstacles();
        ...
        Describe("When a single HDA is selected", [this]()
        {
            ...
            Describe("When randomizing without modifying parameters", [this]()
            {
                ...
                It("Will generate the same parameter every time", [this]()
                {
                    ...
                    float OutSeedNumber;
                    TWeakObjectPtr<UHoudiniPublicAPIAssetWrapper> FirstAsset = Spawner->HoudiniAssetDetails[0].SpawnedActors[0];
        ...
        Describe("When multiple HDA are selected", [this]()
        {
            ...
            Describe("When randomizing without modifying parameters", [this]()
            {
                ...
                It("Will generate the same parameter every time", [this]()
                {
                    ...
                    float OutSeedNumber;
                    TWeakObjectPtr<UHoudiniPublicAPIAssetWrapper> FirstAsset = Spawner->HoudiniAssetDetails[1].SpawnedActors[0];
                    const bool bLoaded = FirstAsset->GetFloatParameterValue("random_seed", OutSeedNumber);
