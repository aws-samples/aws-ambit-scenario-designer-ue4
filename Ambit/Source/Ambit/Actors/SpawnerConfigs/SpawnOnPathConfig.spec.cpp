...
BEGIN_DEFINE_SPEC(SpawnOnPathSpec,
                  "Ambit.SpawnOnPath",
    ...
    Describe("SerializeToJson()", [this]()
    {
        ...
        Describe("the returned JsonObject", [this]()
        {
            ...
            It("JSON has correct fields for Spline Transforms", [this]()
            {
                ...
                SplinePoints.Add(FSplinePoint(1, Tangents, Tangents,
                                              Tangents, Rotation, Scale,
                                              ESplinePointType::Curve));

                Config.SplinePoints = SplinePoints;

                const TSharedPtr<FJsonObject> ConfigJson = Config.
                        SerializeToJson();
                TestTrue("Has SplinePoints field",
                         ConfigJson->HasField("SplinePoints"));

                FSpawnOnPathConfig NewConfig =
                        FSpawnOnPathConfig{};
                ...
                for (int32 i = 0; i < Config.SplinePoints.Num(); i++)
                {
                    ...
                    TestEqual("InputKey", Actual.InputKey,
                              Expected.InputKey);
                    TestEqual("Location", Actual.Position,
                              Expected.Position);
                    TestEqual("ArriveTangent", Actual.ArriveTangent,
                              Expected.ArriveTangent);
                    TestEqual("LeaveTangent", Actual.LeaveTangent,
                              Expected.LeaveTangent);
                    TestEqual("Rotation", Actual.Rotation,
                              Expected.Rotation);
    ...
    Describe("DeserializeFromJson()", [this]()
    {
        ...
        It("sets SplinePoints correctly", [this]()
        {
            ...
            for (const FSplinePoint& Point : Config.SplinePoints)
            {
                if (Point.InputKey == 0)
                {
                    TestEqual("Location", Point.Position,
                              FVector(0));
                }
                if (Point.InputKey == 1)
                {
        ...
        It("when called multiple times, SplinePoints should not accumulate points",
           [this]()
           {
               Config.DeserializeFromJson(Json);
